...
func move_ship(delta: float, avoidance: Vector3, islands: Array[FloatingIsland], measure: bool = false) -> void:
	if not alive:
		return
	...
	velocity = flight.integrate(self, navigation_velocity + avoidance, delta)
	move_and_slide()
...
func _update_attitude(delta: float) -> void:
	...
	var turn_fraction := flight.yaw_velocity / deg_to_rad(yaw_speed_degrees)
