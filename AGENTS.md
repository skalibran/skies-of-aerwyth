...
- Display stretch uses `canvas_items` with `expand` aspect and a 1280 x 800 design resolution. Physics runs at the accepted 30 Hz baseline, with interpolation enabled. Checks and benchmarks use the project physics rate; there is no alternate-rate test matrix or benchmark override.
