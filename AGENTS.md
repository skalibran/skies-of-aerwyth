...
- Combat uses string factions, an inherited Kestrel scene, reddish enemy tint, 500 health, and two tier-one Rusty cannons with authored broadside cones (currently 65° half-angle). Scripted ballistic shots trial 100-unit/s launch speed, gravity 3, range 100, damage 5, eight-second lifetime, and 0.2-second reload for demonstration. Predicted impacts are the default: one delayed target-capsule check with 0.5-unit tolerance; visuals ignore intervening ships/scenery. The original live sweeps remain an explicit comparison mode, where allies intercept without damage. See the [projectile comparison](src/docs/projectile_comparison.md). The debug CombatSpawner adds up to two ships per faction each second, independently replenishing each population to 100 living ships, including the nine starting players. Destroyed ships despawn. See [combat contracts and checks](src/docs/combat.md); the added Kestrel model remains unused.
