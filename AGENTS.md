...
- The main scene is `scenes/world/journey.tscn`: nine player Kestrels using primitive geometry, a moving fleet anchor, local avoidance, a bounded orbit/free camera, floating-origin travel along Z-, and streamed primitive islands. Fleet camera tracking and its viewing sphere use the persistent anchor, so membership changes cannot shift the view through the ship average. There is no on-screen UI in this milestone.
- Combat uses string factions, an inherited Kestrel scene, reddish enemy tint, 500 health, and two tier-one Rusty cannons with authored broadside cones (currently 65° half-angle). Scripted ballistic shots currently trial 100-unit/s launch speed, gravity 3, range 100, damage 5, and an eight-second lifetime; the original 20-unit/s trajectories remain solver reference fixtures. Allies intercept without damage. The debug CombatSpawner adds up to two ships per faction each second, independently replenishing player/enemy populations to 100 living ships each, including the nine starting players. Destroyed ships despawn. See [combat contracts and checks](src/docs/combat.md); the added Kestrel model remains unused.
...
- Endgame combat scale targets roughly 70 friendly ships and 150 enemies. Formation half-extents are 180/120/180 units and the camera sphere radius is 900 units. Starting ships occupy several altitude levels, and local wandering samples all three axes equally. `scripts/tools/check_fleet_scale.gd` exercises 128 ships without combat; `check_combat_scale.gd` measures 220 combat ships and a separate miss-volley workload. Both support rendered profiling with debug drawing off/on; low-end hardware suitability remains unproven.
- Journey owns the combat/travel choice and physics order. Ship avoidance uses a conservative snapshot spatial grid; IslandSpawner caches loaded-island navigation cells and refreshes them on loading, unloading, and rebasing. Exact steering/height tests remain with their existing owners.
- Both factions use FleetController's 180/260-unit combat radii: inward course bias starts at 180, regrouping starts at 260 and ends inside 180, and pursuit excludes targets beyond 260. Flight/avoidance and weapon fire remain active during return. Debug spawns are centered on the persistent anchor, with 100–150-unit horizontal radius and ±150-unit altitude spread. See [combat positioning](src/docs/combat.md#targeting-and-movement).
- ShipFlight supplies primary-axis thrust, retained momentum, lateral drag, and accelerated yaw for the existing CharacterBody3D ships. Combat requests approaches and firing passes through the same controller as travel; it no longer controls hull facing separately. See [flight tuning](src/docs/movement.md#forward-flight-and-momentum) and `scripts/tools/check_ship_flight.gd`.
...
- `scripts/tools/profile_landscape.gd` measures isolated landscape rendering/streaming and compares 1/5/10-unit cells. See the [landscape measurements](src/docs/terrain_performance.md). Terrain collision is not implemented.
...
The [combat and movement review](src/docs/combat_review.md) records the scripted-flight baseline, lifecycle fixes, and CPU measurements. Dense 220-ship combat still exceeds a 60 Hz CPU budget; a functional scale-check PASS is separate from its reported performance result. Combat production gates are COMBAT-01–04 in the combat task list.
