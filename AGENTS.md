...
- Display stretch uses `canvas_items` with `expand` aspect and a 1280 x 800 design resolution. Physics runs at 30 Hz for in-game feel evaluation, with interpolation enabled. The scale benchmark supports process-only 30/60 Hz overrides; legacy reference checks explicitly select 60 Hz.
