...
- Journey instances `scenes/ui/master_ui.tscn`: a full-viewport CanvasLayer layout with three centered, 1920-wide zone scenes (top 140, center 800, bottom 140 logical pixels). Top/bottom pin to viewport edges; center stays centered. Ultrawide windows expose side space, taller aspects create gaps between zones. Red/green/blue translucent test fills ignore input. MasterUI owns viewport placement; feature views fill their hosts. See [UI layout and aspect checks](src/docs/ui.md).
