...
Run the project to open [journey.tscn](../../scenes/world/journey.tscn). It starts with nine player Kestrels using the authored voxel model and balanced palette, a fleet anchor, an orbit camera, and floating islands. The [combat slice](combat.md) adds mounted cannons and debug spawning of ten ships per faction each second, replenishing each side up to 100 living ships. A top-left [performance overlay](../../scenes/ui/performance_overlay.tscn) shows `Ships` (all living ships across both factions, excluding wrecks) and rendered `FPS`. Its timer refreshes twice per second, reading Journey's ship registry and the engine frame rate; the overlay does not intercept input. Production, save files, island management, and cloud transitions are not implemented. Movement checks disable combat and spawning through Journey's validation setting.
