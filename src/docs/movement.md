...
Run the project to open [journey.tscn](../../scenes/world/journey.tscn). It starts with three player Kestrels using the authored voxel model and balanced palette, a fleet anchor, an orbit camera, and floating islands. The [combat slice](combat.md) supplies mounted cannons and combat behavior; normal play has no automatic enemy waves or casualty replenishment. A top-left [performance overlay](../../scenes/ui/performance_overlay.tscn) shows `Ships` (all living ships across both factions, excluding wrecks) and rendered `FPS`. Its timer refreshes twice per second, reading Journey's ship registry and the engine frame rate; the overlay does not intercept input. Production, save files, island management, and cloud transitions are not implemented. Movement checks verify the ordinary three-ship start before disabling combat for their isolated movement fixtures.
