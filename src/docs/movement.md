...
Run the project to open [journey.tscn](../../scenes/world/journey.tscn). It starts with three player Kestrels using the authored voxel model and balanced palette, a fleet anchor, an orbit camera, and floating islands. The [bottom ship picker](ui.md#ship-picker) adds friendly Kestrels on request at random clear positions near the anchor. The [combat slice](combat.md) supplies mounted cannons and combat behavior; normal play has no automatic enemy waves or casualty replenishment. A top-left [performance overlay](../../scenes/ui/performance_overlay.tscn) shows `Ships` (all living ships across both factions, excluding wrecks) and rendered `FPS`. Its timer refreshes twice per second, reading Journey's ship registry and the engine frame rate; the overlay does not intercept input. Production, save files, island management, and cloud transitions are not implemented. Movement checks verify the ordinary three-ship start before disabling combat for their isolated movement fixtures.

The endgame combat target is roughly seventy player ships and 150 enemies. The three starting ships form a compact group, initially about 75-90 meters apart; a separate 128-ship movement check exercises the larger fleet without changing the starting roster, including a solid island placed across its path. The separate combat-scale check includes both fleets and projectiles. Both scale checks explicitly retain their 1800/1200/1800-meter half-extents and 160-meter wander steps independently of the playable starter formation.
...
| --- | --- |
| Formation half-extents, X/Y/Z | 120 / 60 / 120 meters (240 x 120 x 240 overall) |
| Maximum nearby destination step | 40 meters |
| Camera viewing sphere radius | 9000 meters, centered on the persistent fleet anchor |
| Starting orbit distance | 600 meters |
...
| Zoom | 25 meters/scroll step; 300 meters/second on controller, both boosted by sprint |
...
| Voxel ground | 50-meter cubic source grid (trial); adaptive detail over 81 logical 12800-meter regions, Y = -300 to 3400 across biomes |
| Water | Flat scenery at Y = 0; negative terrain forms ponds |

Formation extents, destination step size, and `wander_speed` are exported on FleetController and assigned when each ship joins. The starter anchor cruises at 25 meters/s with 3 meters/s² acceleration; travel corrections relative to it are capped at 8 meters/s. The compact travel volume keeps the three-ship fleet together while nearby goal changes leave room for separation. The starting ships span heights from 3790 to 3810, centered on Y = 3800, and local destination sampling gives X/Y/Z equal weight. Climb speed and attitude limits still keep their motion sluggish. Islands fill a broad surrounding field: X ranges from -16000 to +16000 independently of the formation, and top-reference altitude ranges from 2000 to 5600. The initial field includes both sides, ahead, and behind the fleet. Width, spacing, and altitude are exported on IslandSpawner. If tuning the camera sphere or formation, keep the streaming distances above their combined Z reach with a margin.

The ground is stepped voxel scenery generated with FastNoiseLite, currently trialing 50-meter cubic voxels. TerrainProfile exposes 10/50/100-meter choices. Each patch stays at its logical world position; travel streams terrain rather than sliding it with the fleet. Nearby patches show individual voxels and distant ones simplify the surface. Grassland ranges from -300 to 700 meters, begins blending into broad mountain slopes at 1500 traveled meters, and reaches full mountain terrain at 10000. Mountains span 800 to 3400 meters, with tapered ridges and elevated valleys above sea level. Settings live in a shared profile and separate biome resources. The default region covers the camera sphere plus far range. Maximum terrain height stays below the fleet's cruising altitude but reaches into the lower island band. Full-detail terrain has collision for wrecks; terrain navigation is not implemented. The free camera retains its existing spherical bounds and can pass through scenery. See [terrain authoring and checks](terrain.md).
...
A purple 4-meter cube at `FleetAnchor/AnchorMarker` marks the persistent anchor's exact position. It follows anchor movement and origin shifts, has no collision, and remains visible independently of the F3 navigation overlay.
...
The shared ship scene authors mass 10, friction 0.2, bounce 0, gravity scale 0, and replacement linear/angular damping of 0. ShipFlight owns deliberate drag and lift; engine default damping must not add a second slowdown. Sleeping is disabled for continuously commanded ships. Angular X/Z locks keep the living physical hull upright while the visual child retains limited pitch/bank. The current controller assumes that upright constraint. Changing the shape/mass updates the engine inertia; no duplicate inertia value is maintained.

Death disables all flight-control forces without resetting linear/angular velocity. The wreck uses an authorable gravity multiplier (default 0.2, giving 19.6 meters/s²), default engine damping, free pitch/roll, and sleeping; its collider remains active. It starts the ship's authored square-particle smoke emitters, which stop permanently on first ground contact. It settles on full-detail terrain or islands for three seconds, then freezes while supported and disables its collider. It keeps rebasing until cleanup, 25 seconds after first ground contact (45 seconds after death without landing). Wreck visuals and smoke are hidden over coarse terrain without forcing extra detail. See [death ownership and cleanup](combat.md#ownership-and-cleanup). Water has no surface collision or buoyancy.
...
Kestrel authors the following Airship properties in its scene for both factions. The 50 meters/s maximum leaves catch-up headroom above the 25 meters/s fleet cruise. Softer thrust and yaw build momentum gradually; stronger braking and bounded lateral drag retain course control. The shared primitive ship keeps the reference tuning used by generic controller and fleet-scale fixtures. Tune Kestrel in the Inspector:

| Property | Kestrel value | Effect |
| --- | ---: | --- |
...
| `maximum_speed` / `climb_speed` | 50 / 5 meters/s | Combined propulsion target and separate lift target; neither erases contact impulses. |
| `acceleration` / `braking` | 4 / 6 meters/s² | Forward speed response; acceleration also controls lift response. |
| `yaw_speed_degrees` | 12°/s | Commanded yaw-rate limit; contact-induced spin recovers through bounded torque. |
| `yaw_acceleration_degrees` | 10°/s² | How quickly turning builds and brakes. |
| `lateral_drag` | 0.7/s | Higher values reduce sideways momentum sooner; lower values widen drifting turns. |
...
For an explicitly selected endgame scale check, substitute `res://scripts/tools/check_fleet_scale.gd` in the commands above. It builds 128 ships and a solid island across their path, retains the generic hull's 90 meters/s anchor cruise and 30 meters/s travel correction, simulates 30 seconds, checks detours, hull penetration, formation bounds, progression, rebasing, route-wide island placement, and streaming coverage, and prints median/p95 scripted simulation-step times. Its `--visual` mode also captures the fleet, maximum zoom, and an individual ship. These timings exclude native rigid-body integration/contact solving, debug drawing, and the rest of frame rendering; they are not an endgame frame-rate guarantee with combat and production. Endgame performance work is tracked under GEN-05 in the [generation task list](todo/blockers-terrain.txt).
