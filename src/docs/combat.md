...
The slot origin is the equipment origin, muzzle, and cone apex. The current authored Kestrel tuning places cannons beside the gondola at local X = +/-14, Y = -13, Z = 3 meters, with a **65-degree half-angle (130-degree opening)**. The visual barrel pivots around the muzzle without rotating the allowed cone. Visual pitch/bank carries both slots with the ship.
