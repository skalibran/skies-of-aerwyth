...
Run `scenes/world/journey.tscn` for a starting fleet of three player Kestrels. Normal play has no automatic combat spawns, enemy waves, or casualty replenishment. Combat remains enabled for registered hostile ships. Destroyed ships become passive falling wrecks, freeze after three seconds of supported settling, and are removed 25 seconds after first ground contact. Production, purchased reinforcements, difficulty progression, defeat screens, water-specific wreck behavior, and combat UI remain unimplemented.

The opt-in extended combat check creates and steps [`CombatSpawner`](../../scripts/tools/combat_spawner.gd) explicitly. This validation helper adds up to ten ships per faction each second and independently replenishes each side to 100 living ships, counting the starting players. Partial batches never exceed the cap, and capped or blocked ticks do not accumulate a later burst. The playable Journey has no spawner node or spawning callback.
