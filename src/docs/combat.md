...
The [combat and movement review](combat_review.md) preserves the CharacterBody3D baseline; the [rigid-body integration](rigid_body_trial.md) records the accepted controller, comparison, and replacement audit. The [projectile comparison](projectile_comparison.md) records the subsequent delayed-hit trial at ten times the firing rate. Outstanding combat work lives in the [combat task list](todo/todo-combat.txt); controller acceptance is recorded in the [movement task list](todo/todo-movement.txt).
...
| Reload per cannon | 0.2 seconds (5 shots/s, tenfold demonstration rate) |
...
[`ProjectileController`](../../scripts/combat/projectile_controller.gd) advances all shots centrally using the constant-acceleration equation. Each shot owns local position, velocity, gravity, lifetime, damage, captured source faction, and one shared-mesh visual. No rigid bodies, projectile areas, or per-shot timers are used. **Hit Model** on Journey's `Projectiles` node chooses the model for newly fired shots; existing shots retain their launch-time model.

**Predicted Impact is the default trial.** The mounted weapon returns its launch velocity and interception time together, without solving twice. Firing captures the intended target weakly, the arrival time, and the expected point on the same ballistic arc. On the first physics tick reaching that time, the controller checks that point against the target's current, oriented capsule, expanded by **Impact Tolerance** (default 0.5 units, captured at launch). A living, still-hostile target inside that tolerance takes damage and the visual disappears. Target movement after launch never changes the snapshot or bends the arc.

A failed check is final. The visual continues along its gravity arc until the eight-second lifetime, without hitting another ship later. A removed, destroyed, or newly non-hostile target also becomes a miss. Untargeted shots are visual-only. These projectiles make **no collision queries**: allies, other opponents, and islands cannot intercept them. Terrain and water already have no collision. This deliberately changes combat rules; a ball may visibly pass through an intervening hull or island. Arrival checks use the latest completed target pose, with timing quantized to one physics tick; the tolerance is a gameplay allowance, not exact continuous collision. The shooter can disappear without cancelling its shot.

**Live Sweep is retained for comparison.** Each physics step sweeps the short arc chord through Godot's physics space. A curvature bound of `gravity * dt² / 8` subdivides unusually long steps to keep chord error below 0.01 units. At 60 Hz, one ray per shot suffices. The first collision consumes the shot. An opposing ship takes damage; allied ships and island colliders consume it without damage. Only the firing ship is excluded, using a weak reference. Shots can hit unintended ships, including beyond targeting range, and are independent of the original target. These are point sweeps against the current physics snapshot, not continuous collision between moving shapes.

Both models use targeting distance rather than arc length for range, and clip advancement to the remaining lifetime. Neither homes, applies splash damage, or changes the ballistic solver. The same MeshInstance3D-per-shot presentation is deliberately retained for the comparison; this trial does not aggregate minigun bursts or batch visuals.
...
Force submission does not immediately move the hull. Godot/Jolt integrates rigid-body motion and resolves contacts for the next physics snapshot. Avoidance, aiming, and projectile checks read the latest completed body state; aiming and debug velocity use the body's `linear_velocity`. Hull X/Z rotation is locked while the visual child banks/pitches. Ship control does not overwrite physical transforms or velocities each frame; projectile motion remains scripted in either hit model.
...
The translation-only projectile root is registered once with FloatingOrigin. Shots store both current and expected impact positions relative to that root. Delayed checks and live ray endpoints use its current transform. A rebase changes neither velocity nor gravity/lifetime and never creates a long sweep or invalidates the impact snapshot. Scene ownership releases projectiles, visuals, and subscriptions on exit; `clear()` also cancels pending checks.
...
```text
--headless --path <absolute-project-path> --script res://scripts/tools/check_predicted_projectiles.gd --log-file <external-log-file>
--headless --path <absolute-project-path> --fixed-fps 60 --script res://scripts/tools/check_combat.gd --log-file <external-log-file>
```

`check_predicted_projectiles.gd` covers gravity at 20/100-unit launch speeds with 30/60/120 Hz projectile steps, elevated moving targets, immutable snapshots, delayed damage, misses continuing without rechecking, rotated capsule tolerance, intentional non-interception, lifetime boundaries, zero/large steps, target/shooter removal, simultaneous lethal arrivals, rebasing, and cleanup. The fixtures advance controlled target trajectories explicitly. `check_combat.gd` retains live-sweep impact fixtures and exercises the ordinary journey with the default predicted model.
...
[`check_combat_scale.gd`](../../scripts/tools/check_combat_scale.gd) runs seventy player Kestrels and 150 enemies, with 1,000,000 benchmark-only health and scheduled deaths/replacements to sustain the same population under either hit rule. It disables the ordinary debug spawner and includes debug-off/on phases, streaming/camera movement, origin shifts, and a separate synchronized miss-volley phase reaching 1,760 live shots. The battle uses the authored 0.2-second reload. The historical miss fixture deliberately keeps 440 shots every two seconds, launched without targets 1,000 units above ships to clear scenery. It isolates repeated collision queries versus visual-only flight; it does not measure a tenfold miss storm or delayed target checks. These settings are independent of the ordinary scene's 500 health and 100-per-faction caps.
...
Append `--live-projectiles` to the user arguments to run the retained swept model. Use a separate output directory per run. Both modes otherwise use identical weapon tuning, fleet layout, and visuals. Current results and comparison limits are recorded in [projectile comparison](projectile_comparison.md); the older measurements below retain their original workloads.
