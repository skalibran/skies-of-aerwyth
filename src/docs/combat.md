...
One world unit is one meter; see the [unit conversion](world_units.md). Both factions spawn 1000–1500 horizontal meters from the persistent fleet anchor, within 1500 meters above or below its altitude (a 3000-meter vertical band). Tune **Altitude Spread** on CombatSpawner. The ship average and camera position do not move this spawn region. Origin shifts preserve its position relative to the anchor. Default spawn positions remain inside the 2600-meter combat radius.

## Authored ships and weapons

[`kestrel.tscn`](../../scenes/ships/kestrel.tscn) inherits the shared [`ship.tscn`](../../scenes/ships/ship.tscn), replaces its primitive visuals with the authored voxel model, and fits the capsule to that model. It retains movement values, owns two tier-one slots, assigns their starting equipment, and authors its combat directions. Both factions instantiate the same scene. Set `entity_id` and `faction` **before adding the ship to the tree**, because readiness initializes weapon phases and tint. Then position the ship and register it with Journey. `player` and `enemy` are hostile to each other; matching identifiers are allied, and other strings are non-hostile. The existing neutral navigation fixtures remain supported. Live faction switching is not part of this slice.
