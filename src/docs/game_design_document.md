...
Cannonballs follow gravity-driven ballistic arcs, with weapons calculating elevation and lead for moving targets before firing. The current trial captures the expected impact position and travel time at launch. A visual ball follows that same arc without collision queries. At arrival, one check against the intended target's current hull, with 0.5 units of tolerance, deals damage and removes the ball on success. Misses continue falling until lifetime expiry and never check again. Missing, dead, or non-hostile targets produce misses. Allies, other enemies, and scenery do not intercept these shots; the original live-sweep model remains available for comparison. The slot cone constrains the initial launch direction, including elevation and lead. Rusty cannon favors the earliest intercept and a low arc, using authorable stylized gravity that permits its 100-unit level shot at the specified launch speed. Range measures muzzle-to-target distance, while an independent lifetime allows the longer curved flight; targets within range can still be ballistically unreachable. Current tuning and the comparison are recorded in [combat notes](combat.md) and [projectile measurements](projectile_comparison.md).

The current debug spawner adds two ships every simulation second for each faction, independently capped at one hundred living player ships and one hundred living enemies. Count the starting ships toward their faction's cap. Replenish casualties on later scheduled ticks at the same rate, filling only available capacity; debug spawning continues after a faction is wiped out. There is no encounter difficulty curve or combat progression in this slice. Rusty cannons currently fire up to five shots per second each (0.2-second reload), ten times their original cadence, for the projectile demonstration and comparison. Friendly fire deals no damage in either projectile model; only live sweeps are consumed by friendly interception. Impacts have no visual explosion effect, and destroyed ships simply despawn. Wrecks, production, player-directed reinforcements, and the visual ship editor remain later work. Existing journey and terrain progression continues independently.
